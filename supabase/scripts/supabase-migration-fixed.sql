-- =============================================
-- AGROAFRICA DATABASE MIGRATION (FIXED)
-- Run this in your Supabase SQL Editor
-- This version handles the crop_id error
-- =============================================

-- =============================================
-- STEP 0: CLEAN UP PROBLEMATIC OBJECTS
-- Remove any old functions/triggers that reference crop_id
-- =============================================

-- Drop any functions that might reference crop_id
DO $$
DECLARE
    func_record RECORD;
BEGIN
    ...
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS public.' || quote_ident(func_record.function_name) || ' CASCADE';
        RAISE NOTICE 'Dropped function: %', func_record.function_name;
    END LOOP;
END $$;

-- Drop any policies that might reference crop_id
DO $$
DECLARE
    pol_record RECORD;
BEGIN
...
DECLARE
    pol_record RECORD;
    tables TEXT[] := ARRAY['fields', 'expenses', 'income', 'tasks', 'inventory', 'storage_bins', 'community_posts', 'marketplace_listings'];
    tbl TEXT;
BEGIN
    ...
    LOOP
        FOR pol_record IN
            ...
            WHERE tablename = tbl
        LOOP
            EXECUTE 'DROP POLICY IF EXISTS ' || quote_ident(pol_record.policyname) || ' ON ' || quote_ident(tbl);
        END LOOP;
    END LOOP;
END $$;
...
-- 1. Cleans up any old functions/policies referencing crop_id
-- 2. Adds user_id column to all user-specific tables
-- 3. Creates indexes for fast queries
-- 4. Enables Row Level Security (RLS)
-- 5. Creates strict policies so users only see their own data
-- 6. Creates a user_profiles table for storing preferences
-- 7. Sets up automatic profile creation on signup
--
